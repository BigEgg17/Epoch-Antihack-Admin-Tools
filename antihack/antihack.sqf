...
private [
	...
	"_kfc_akey", "_kfc_strd", "_kfc_info", "_kfc_keys", "_kfc_gclnt", "_kfc_sclnt", "_kfc_nclnt", "_kfc_rekey", "_kfc_atrd_cfg", "_kfc_strd_cfg"
];
...
_AH_Global = _AH_Global + ("
	'"+_kfc_rekey+"' addPublicVariableEventHandler {
		...
		if (_id == 6) exitWith {
			player setVariable [Z_BankVariable, (player getVariable [Z_BankVariable, 0]) + (_param select 1), true];
			call player_forceSave;
			[format['%1 %2 were added to your bank by admin ""%3.""', [_param select 1] call BIS_fnc_numberText, CurrencyName, _info], 4] call "+_kfc_msg+";
		};
		if (_id == 7) exitWith {
			player setVariable [Z_MoneyVariable, (player getVariable [Z_MoneyVariable, 0]) + (_param select 1), true];
			call player_forceSave;
...
_AH_Admin = _AH_Admin + ("
	...
	AH_fnc_open = {
		...
		if (admin_adjcoins) then {
			(_display displayCtrl 1405) ctrlSetEventHandler ['KeyDown', ""
				if ((_this select 1) == 0x1C) then {
					[parseNumber ctrlText(_this select 0), Z_MoneyVariable, 'wallet'] call AH_fnc_adjust;
				};
			""];
		};
		if (admin_adjbank) then {
			(_display displayCtrl 1406) ctrlSetEventHandler ['KeyDown', ""
				if ((_this select 1) == 0x1C) then {
					[parseNumber ctrlText(_this select 0), Z_BankVariable, 'bank'] call AH_fnc_adjust;
				};
			""];
		};
	...
	AH_fnc_selTarget = {
		...
		[
			...
			[(_tar getVariable [Z_moneyVariable, 0])] call BIS_fnc_numberText,
			[(_tar getVariable [Z_bankVariable, 0])] call BIS_fnc_numberText,
			getPosATL _tar,
	...
	AH_fnc_target = {
		...
		if (_this == 'Adjust Target Bank') exitWith {
			...
			"", _name, _puid], 'Adjust', format['Target: %1 (%2)', _name, _puid], format['Current: %1', [_target getVariable [Z_bankVariable, 0]] call BIS_fnc_numberText]] call AH_fnc_display;
		};
		if (_this == 'Adjust Target Wallet') exitWith {
			...
			"", _name, _puid], 'Adjust', format['Target: %1 (%2)', _name, _puid], format['Current: %1', [_target getVariable [Z_moneyVariable, 0]] call BIS_fnc_numberText]] call AH_fnc_display;
		};
			...
			if (isNil 'admin_specTarget') then {
				...
				"+_kfc_atrd_cfg+" set [count "+_kfc_atrd_cfg+", [{
					...
					[format[""<t size='0.5'>%1 (%2)</t><br/><t size='0.5'>Bank: %3 - Wallet: %4 - Blood: %5 - Humanity: %6</t><br/><t size='0.5'>Position: %7 - Weapon: %8</t>"",
						...
						[admin_specTarget getVariable [Z_BankVariable, 0]] call BIS_fnc_numberText,
						[admin_specTarget getVariable [Z_MoneyVariable, 0]] call BIS_fnc_numberText,
	...
	admin_info = {
		...
		_coins = _ct getVariable [Z_MoneyVariable, -1];
